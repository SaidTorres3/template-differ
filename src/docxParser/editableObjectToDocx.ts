...
export const editableObjectToDocx = async (opts: EditableObjectToDocxOpts) => {
  return new Promise((resolve, reject) => {
    const docxFile = fs.readFileSync(opts.fileInPath);
    // unzip the file
      ...
      file.async('text').then(function (XMLContent) {
        xml2js.parseString(XMLContent, function (err, result) {
          ...
          zipContent.generateAsync({ type: 'base64' }).then(function (outputFile) {
            fs.writeFileSync(opts.fileOutPath, outputFile, 'base64');
          })
        });
      })
    });
