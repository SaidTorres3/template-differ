...
export const editableObjectToDocx = async (
  opts: EditableObjectToDocxOpts
): Promise<Buffer> => {
        ...
        xml2js.parseString(XMLContent, function (err, result) {
          ...
          paragraphs.forEach(
            (
              ...
              wRLabels.forEach(
                (wRLabel: { [x: string]: any[] }, wRLabelIndex) => {
                  const modfiedPhrase = getModifiedPhare(opts.modifiedObjects, {
                    paragraphIndex,
                    sentenseIndex: wRLabelIndex,
                  });
                  if (!modfiedPhrase) {
                    return;
                  }
...
const getModifiedPhare = (
  phrases: Phrase[],
  phaseCoords: PhraseCoords
): Phrase | undefined => {
  const phrase = phrases.find((phrase: Phrase) => {
    if (
      phrase.paragraphIndex === phaseCoords.paragraphIndex &&
      phrase.sentenseIndex === phaseCoords.sentenseIndex
    ) {
      return phrase;
    }
  });
